    ...
    if wparam.0 == WM_KEYDOWN as usize || wparam.0 == WM_SYSKEYDOWN as usize {
        ...
        if let Some(ch) = vk_to_char(vk) {
            TYPED_BUFFER.with(|buf| {
                ...
                // Check for hotstring and prompt matches
                HOOK_CONFIG.with(|hc| {
                    let hc = hc.borrow();
                    if let Some(ref cfg_arc) = *hc {
                        if let Ok(cfg) = cfg_arc.lock() {
                            // Check hotstrings first (higher priority)
                            for hs in &cfg.hotstrings {
                                if buf.ends_with(&hs.trigger) {
                                    ...
                                    std::thread::spawn(move || {
                                        expand_hotstring(&expansion, trigger_len, replace);
                                    });
                                    return; // exit closure — match found
                                }
                            }

                            // Check prompts as slash-commands: /name followed by space
                            if buf.ends_with(' ') {
                                let trimmed = &buf[..buf.len() - 1]; // without trailing space
                                if let Some(slash_pos) = trimmed.rfind('/') {
                                    let cmd = &trimmed[slash_pos + 1..];
                                    for prompt in &cfg.prompts {
                                        if prompt.name.to_lowercase() == cmd {
                                            info!("Prompt triggered: /{}", prompt.name);
                                            let expansion = prompt.template.clone();
                                            // trigger length = /name + space
                                            let trigger_len = prompt.name.len() + 2;

                                            buf.clear();

                                            std::thread::spawn(move || {
                                                expand_hotstring(&expansion, trigger_len, true);
                                            });
                                            return; // exit closure — match found
                                        }
                                    }
