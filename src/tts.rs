...
fn enumerate_from_path(path: &str, hive: &str) -> Result<Vec<VoiceInfo>, String> {
    ...
    unsafe {
        ...
        loop {
            ...
            // End of enumeration: S_FALSE or zero fetched
...
fn speak_sapi(text: &str, voice: &VoiceInfo, speed: i32, volume: u32) {
    ...
    // Set voice by token ID using SAPI COM. ISpeechObjectToken::SetId
    // (automation/IDispatch interface) takes Id as the first parameter,
    // CategoryID as optional second (reversed from ISpObjectToken::SetId).
    let voice_setup = if voice.id.is_empty() {
        // No specific voice token — use system default
        String::new()
    } else {
        format!(
