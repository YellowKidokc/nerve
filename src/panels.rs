...
impl PanelManager {
    ...
    fn send_to_panel(&self, panel_name: &str, json_payload: &str) {
        if let Some(panel) = self.windows.get(panel_name) {
            let payload_literal = serde_json::from_str::<serde_json::Value>(json_payload)
                .ok()
            ...
            let script = format!(
                ...
                (() => {{
                  ...
                  if (payload && payload.type === 'config') {{
                    if (payload.config && typeof window.config !== 'undefined') window.config = payload.config;
                    if (typeof window.populateUI === 'function') window.populateUI();
                  }}

                  if (payload && payload.type === 'clips') {{
                    if (Array.isArray(payload.clips) && typeof window.clips !== 'undefined') window.clips = payload.clips;
                    if (typeof window.renderSlots === 'function') window.renderSlots();
                    if (typeof window.renderClips === 'function') window.renderClips();
                  }}

                  if (payload && payload.type === 'voices' && typeof window.populateVoices === 'function') {{
                    window.populateVoices(payload.voices || []);
                  }}
                }})();
                "#,
